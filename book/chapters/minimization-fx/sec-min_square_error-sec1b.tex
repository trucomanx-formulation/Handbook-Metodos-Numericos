...
as matrizes diagonais $\MATRIX{C} \in \mathbb{R}^{M\times M}_{+}$ e
$\MATRIX{D} \in \mathbb{R}^{N\times N}_{+}$, e 
...
devemos usar\footnote{A demonstração pode ser vista na Prova \ref{proof:theo:minAxbCAxbalphaxqD}.} a Eq. (\ref{eq:minAxbCAxb2alphaxqD}),
...
\end{equation}
Assim, o mínimo $\VECTOR{\hat{x}}$ existe e é único só se a 
matriz $\MATRIX{A}^{\transpose}\MATRIX{C}\MATRIX{A} +\alpha\MATRIX{D}$ tem inversa.
\end{theorem}
...
as matrizes $\MATRIX{A} \in \mathbb{R}^{3\times 2}$, $\MATRIX{C} \in \mathbb{R}^{3\times 3}_{+}$  
e $\MATRIX{D} \in \mathbb{R}^{2\times 2}_{+}$,
o escalar $\alpha \in \mathbb{R}$,
...
Com todos esses dados e usando a Eq. (\ref{eq:minAxbCAxb1alphaxqD}),
obtemos a superfície $e(\VECTOR{x})$, como mostra a Figura \ref{fig:ex:minAxbCAxb1alphaxqD:a}.
Usando a Eq. (\ref{eq:minAxbCAxb2alphaxqD}), 
calculamos o ponto $\VECTOR{\hat{x}}=[0.85714\quad 0.85714]^{\transpose}$
que minimiza a Eq. (\ref{eq:minAxbCAxb1alphaxqD}), com um $e(\VECTOR{\hat{x}})=0.97714$.
