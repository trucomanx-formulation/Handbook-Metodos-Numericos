...
com um erro $e(\VECTOR{\hat{x}})=1.4956~10^{-06}\approx 0$ e uma pendente\footnote{\label{ref:pendenteex} O
cálculo da pendente de $e(\VECTOR{\hat{x}})$ pode ser vista no Teorema \ref{theo:derfxbCfxb0}.}
$\frac{\partial e(\VECTOR{\hat{x}})}{\partial \VECTOR{x} }=[3.8742~10^{-12}\quad 3.8742~10^{-12}]^{\transpose}$
próxima a zero;
...
$e(\VECTOR{x}_k)$ & 1.5101e+00   & 5.3143e-01   & 8.4175e-02   & 2.1957e-03   & 1.4956e-06 \\ \hline
\end{tabular}
...
com pendente $\frac{\partial e(\VECTOR{x}_0)}{\partial \VECTOR{x} }=[-2~-2]^{\transpose}$,
...
com um erro $e(\VECTOR{\hat{x}})=0.63391$; é dizer, não existe $\VECTOR{f}(\VECTOR{\hat{x}})$
que seja igual a $\VECTOR{b}$, só podemos achar um vetor $\VECTOR{\hat{x}}$ 
que provoque um valor com mínimo erro, com pendente 
$\frac{\partial e(\VECTOR{\hat{x}})}{\partial \VECTOR{x} }=[-4.8666~10^{-6}\quad -4.8666~{10}^{-6}]^{\transpose}$
e um
\begin{equation}
...
$e(\VECTOR{x}_k)$ & 1.38223   & 0.67949   & 0.63403   & 0.63391   & 0.63391   & 0.63391 \\ \hline
\end{tabular}
...
\begin{example}[Quando existem multiples mínimos locais e um
$\VECTOR{\hat{x}}$ que cumpre que $\VECTOR{f}(\VECTOR{\hat{x}}) \approx \VECTOR{b}$:]
...
com pendente\footnote{\label{ref:pendenteex3} O cálculo da
pendente de $e(\VECTOR{\hat{x}})$ pode ser visto no Teorema \ref{theo:derfxbCfxb0}.} 
$\frac{\partial e(\VECTOR{x}_0)}{\partial \VECTOR{x} }=[2.1159~10^{-4}\quad 2.1159~10^{-4}]^{\transpose}$ e 
usamos iterativamente a Eq. (\ref{eq:minfxbCfxb2}), obtemos os valores 
...
com um erro $e(\VECTOR{\hat{x}})=1.208$ e uma pendente
$\frac{\partial e(\VECTOR{\hat{x}})}{\partial \VECTOR{x} }=[0.10125\quad 0.10125]^{\transpose}$
pequena e um
\begin{equation}
\MATRIX{J}(\VECTOR{\hat{x}})=
\begin{bmatrix}
...
$e(\VECTOR{x}_k)$ & 1.9293   & 1.9293   & 1.9293   & 1.8726   & 1.5239   & 1.2099   & 1.2087   & 1.2080 \\ \hline
\end{tabular}
...
com pendente\footnote{\label{ref:pendenteex3} O cálculo da
pendente de $e(\VECTOR{\hat{x}})$ pode ser visto no Teorema \ref{theo:derfxbCfxb0}.} 
$\frac{\partial e(\VECTOR{x}_0)}{\partial \VECTOR{x} }=[3.8171~10^{-5}\quad 3.8171~10^{-5}]^{\transpose}$ e 
usamos iterativamente a Eq. (\ref{eq:minfxbCfxb2}), obtemos os valores 
...
com um erro $e(\VECTOR{\hat{x}})=0.13883$ e uma pendente
$\frac{\partial e(\VECTOR{\hat{x}})}{\partial \VECTOR{x} }=[-0.020477\quad -0.020477]^{\transpose}$
pequena e um
\begin{equation}
\MATRIX{J}(\VECTOR{\hat{x}})=
\begin{bmatrix}
...
$e(\VECTOR{x}_k)$ & 1.71499   & 1.71499   & 1.71498   & 1.67228   & 1.28276   & 0.49716   & 0.26843   & 0.13883 \\ \hline
\end{tabular}
\caption{Resposta iterativa do Exemplo \ref{ex:minfxbCfxb3}.}
