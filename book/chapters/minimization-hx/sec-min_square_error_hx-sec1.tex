...
\textbf{Considerações:}
\begin{itemize} 
%\item Se sabemos que é possível a igualdade $h(x)=a$,
%um critério de parada para a Eq.  (\ref{eq:minhxhx2}), seria verificar
% $||h(x_{k-1})-a||^2<\varepsilon^2$,
%sendo $\varepsilon$ é um valor pequeno escolhido por nós;
...
Conhecida uma função $h(x)=x^2$ é valor $a=1$ do contradominio de $h(x)$,
...
Neste caso a Eq. (\ref{eq:minhxhx2}) converge sem problemas em $\hat{x}\approx x_4=-1$ com $e(\hat{x})\approx 0$.
...
Neste caso a Eq. (\ref{eq:minhxhx2}) diverge 
...
Conhecida uma função $h(x)=x^3$ é valor $a=1$ do contradominio de $h(x)$,
achar o valor $\hat{x}$ que minimize $e(x)=(h(x)-a)^2$.
\end{example}
...
pode que seja possível atingir o minimo de $e(x)$.
\end{SolutionT}
