...
TEXTO=['$\hat{y}$'];%= [' num2str(round(10*Bh(1))/10) ';' ];
ht2=text(   Bh(1),Bh(2)-0.5, Bh(3), TEXTO);
...
print(gcf,'surfcax.tex','-depslatexstandalone','-tight',['-F:' num2str(FONTSIZE)])
...
set(hz,'fontsize',1.5*FONTSIZE);
hx1=xlabel('$x_1$');
set(hx1,'fontsize',FONTSIZE);
hx2=ylabel('$x_2$');
set(hx2,'fontsize',FONTSIZE);
colormap(jet)
TEXTO=['$\hat{x}$'];%=[' num2str(round(P(1)*100)/100) ';' num2str(round(P(2)*100)/100)  ']'];
