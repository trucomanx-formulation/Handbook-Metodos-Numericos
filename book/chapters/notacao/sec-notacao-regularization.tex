...
onde é desenhada a superfície $e(\VECTOR{x})$ em relação ao ponto mínimo $\VECTOR{\hat{x}}$.
...
podemos usar o Teorema \ref{theo:minAxbCAxbplusalphaxqD}, de modo que o vetor $\VECTOR{x}=\VECTOR{\hat{x}}$ que minimiza $e(\VECTOR{x})$
é igual a
...
o ponto $\VECTOR{\hat{x}}$ não esta no vale da superfície $||\MATRIX{A}\VECTOR{x}-\VECTOR{y}_{\delta}||^2$;
