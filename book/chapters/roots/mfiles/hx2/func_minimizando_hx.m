function hf=func_minimizando_hx(X1,M,DAT)
    X=zeros(1,M);
    H=zeros(1,M);
    E=zeros(1,M);
    X(1)=X1;
    ...
    for II=2:M
        X(II)=X(II-1)-funh0(X(II-1))/funh1(X(II-1));
    ...
    for II=1:(M-1)
        ...
        quiver (X(II), funh0(X(II)), X(II+1)-X(II), DELTAE,'color','k','linewidth',DAT.LINEWIDTH,'maxheadsize', 0.1);
    end
    hold off
    set(gca, "fontsize", DAT.FONTSIZE);
end
